...
- **ModredIP (Lectern)**: `0x5829940874605d61496CE818914B972c507E55c7`
- **Contract Key**: `ModredIPModule#ModredIP` (maintained for compatibility)
- **Status**: ✅ Active and verified to have `registerIP` function

**Note**: The contract key name "ModredIPModule#ModredIP" is maintained for backward compatibility, but the application is branded as "Lectern".
...
2. **After deployment**, update `app/src/deployed_addresses.json` (or run `yarn install` in `app/` to copy from `ignition/deployments/chain-420420417/`):
   ```json
   {
   ...
   **Note**: The key "ModredIPModule#ModredIP" is maintained for compatibility, but the application name is "Lectern".

3. **Verify the contract** (optional):
   ```bash
   npx hardhat verify --network polkadotHubTestnet DEPLOYED_ADDRESS "REGISTRY_ADDRESS" "ACCOUNT_IMPL_ADDRESS" 420420417 "PLATFORM_FEE_COLLECTOR"
   ```

### Deployment Steps

1. Make sure you have PAS tokens in your deployer wallet for gas fees
...
1. Visit: https://blockscout-testnet.polkadot.io/address/CONTRACT_ADDRESS
