...
const mintLicense = async (licenseRequest) => {
    try {
        ...
        const hasExistingLicense = await (0, storyService_1.checkExistingLicenses)(licenseRequest.tokenId, licenseRequest.contractAddress);
        ...
        const { txHash, blockNumber, explorerUrl } = await (0, storyService_1.mintLicenseOnChain)(licenseRequest.tokenId, licenseRequest.royaltyPercentage, licenseRequest.duration, licenseRequest.commercialUse, licenseRequest.terms, licenseRequest.contractAddress);
        const result = {
            success: true,
            txHash,
            blockNumber,
            explorerUrl,
            message: 'License minted successfully on Polkadot Hub'
        };
        ...
        return {
            ...
            message: 'Failed to mint license on Polkadot Hub'
        };
    }
};
