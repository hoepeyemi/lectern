...
import { Address, Hash, parseGwei } from 'viem';
...
export const registerIpOnChain = async (
    ipHash: string,
    metadata: string,
    isEncrypted: boolean,
    modredIpContractAddress: Address
    ...
    try {
        ...
        try {
            ...
            hash = await walletClient.writeContract({
                ...
                maxPriorityFeePerGas: parseGwei(MIN_PRIORITY_FEE_GWEI),
            });
        ...
        if (!hash) {
            ...
            if (hash) {
                const RECEIPT_TIMEOUT_MS = 300_000; // 5 minutes
                try {
...
export const mintLicenseOnChain = async (
    tokenId: number,
