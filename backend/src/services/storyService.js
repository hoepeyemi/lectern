...
exports.mintLicenseOnChain = exports.registerIpOnChain = exports.checkExistingLicenses = void 0;
const config_1 = require("../utils/config");
const viem_1 = require("viem");
// Polkadot Hub Testnet requires a minimum priority fee (RPC rejects "Priority is too low" otherwise)
...
const registerIpOnChain = async (ipHash, metadata, isEncrypted, modredIpContractAddress) => {
    const maxRetries = 3;
    let lastError = null;
    for (let attempt = 0; attempt < maxRetries; attempt++) {
        try {
            ...
            let hash;
            try {
                await config_1.publicClient.simulateContract({
                    address: modredIpContractAddress,
                    abi: MODRED_IP_ABI,
                ...
                hash = await config_1.walletClient.writeContract({
                    ...
                    maxPriorityFeePerGas: (0, viem_1.parseGwei)(MIN_PRIORITY_FEE_GWEI),
                });
            ...
            // If we have a hash, wait for receipt (long timeout for slower chains like Polkadot Hub)
            if (hash) {
                const RECEIPT_TIMEOUT_MS = 300000; // 5 minutes
                try {
                    ...
                    const isTimeout = (receiptError?.message || '').includes('Timed out') ||
                        receiptError?.name === 'WaitForTransactionReceiptTimeoutError';
                    if (isTimeout) {
                        // Tx was submitted; return success with hash so user can check explorer
                        console.warn(`⏳ Receipt wait timed out. Transaction was submitted: ${hash}`);
                        return {
                            txHash: hash,
                            ipAssetId: undefined,
                            blockNumber: null,
...
exports.registerIpOnChain = registerIpOnChain;
const mintLicenseOnChain = async (tokenId, royaltyPercentage, duration, commercialUse, terms, modredIpContractAddress) => {
    const maxRetries = 3;
    let lastError = null;
    for (let attempt = 0; attempt < maxRetries; attempt++) {
        try {
            ...
            // If we have a hash, wait for receipt (long timeout for slower chains)
            if (hash) {
                const RECEIPT_TIMEOUT_MS = 300000; // 5 minutes
                try {
                    const receipt = await config_1.publicClient.waitForTransactionReceipt({
...
exports.mintLicenseOnChain = mintLicenseOnChain;
