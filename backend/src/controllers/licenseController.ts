...
const handleLicenseMinting = async (req: Request, res: Response) => {
    ...
    try {
        const { tokenId, royaltyPercentage, duration, commercialUse, terms, lecternContractAddress } = req.body;
        const contractAddress = lecternContractAddress;
...
if (!tokenId || !royaltyPercentage || !duration || commercialUse === undefined || !terms || !contractAddress) {
      return res.status(400).json({
        error: 'Missing required parameters: tokenId, royaltyPercentage, duration, commercialUse, terms, lecternContractAddress'
      });
    }
