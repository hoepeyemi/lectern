...
const handleRegistration = async (req, res) => {
    ...
    try {
        ...
        if (!contractAddress) {
            return res.status(400).json({
                error: 'Missing required parameter: lecternContractAddress (or modredIpContractAddress). Set skipContractCall=true to test without contract.'
            });
        }
        // 1. Register on Polkadot Hub using Lectern (ModredIP) contract
        let txHash = null;
        let ipAssetId = undefined;
        let blockNumber = null;
        let explorerUrl = null;
        try {
            const result = await (0, storyService_1.registerIpOnChain)(ipHash, metadata, isEncrypted, contractAddress);
            if (!result)
            ...
            console.log("✅ Polkadot Hub registration successful:", {
                txHash,
                ipAssetId,
                blockNumber,
                explorerUrl
        ...
        // 2. Submit to Yakoa (if ipAssetId and blockNumber are available; skip when receipt timed out)
        if (ipAssetId && blockNumber !== null && txHash) {
            ...
            const authorizations = [
                {
                    ...
                    data: {
                        type: 'email',
                        email_address: parsedMetadata.creator_email || 'creator@lectern.com'
                    }
                }
            ];
            const yakoaResponse = await (0, yakoascanner_1.registerToYakoa)({
                ...
                brandId: null,
                brandName: null,
                emailAddress: parsedMetadata.creator_email || 'creator@lectern.com',
                licenseParents: [],
                authorizations: authorizations,
            });
            // Determine success message based on Yakoa response
            const successMessage = yakoaResponse.alreadyRegistered
                ? 'IP Asset registered on Polkadot Hub, already exists in Yakoa'
                : 'IP Asset successfully registered on Polkadot Hub and Yakoa';
            const responseData = {
            ...
            // No ipAssetId, or receipt timed out (blockNumber null) — return success with tx hash when available
            const responseData = {
                message: txHash
                    ? (blockNumber === null ? 'Registration submitted. Receipt is pending — check the explorer to confirm.' : 'Registration successful (IP Asset ID not extracted)')
                    : 'Registration successful (IP Asset ID not extracted)',
                polkadotHub: {
                    txHash,
