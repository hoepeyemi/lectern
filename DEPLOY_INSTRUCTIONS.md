...
- **Explorer (Routescan):** https://polkadot.testnet.routescan.io/  
...
2. **Ensure you have PAS tokens** in your deployer wallet for gas fees on Polkadot Hub Testnet
...
**PowerShell:**
```powershell
npx hardhat ignition deploy ignition/modules/ModredIP.ts --network polkadotHubTestnet
```
...
2. **Update addresses:** Either copy `ignition/deployments/chain-420420417/deployed_addresses.json` to `app/src/deployed_addresses.json`, or run `yarn install` in the `app/` folder (postinstall copies it). Or edit `app/src/deployed_addresses.json`:
   ```json
   {
...
1. Visit: https://blockscout-testnet.polkadot.io/address/YOUR_NEW_CONTRACT_ADDRESS
...
1. **Wait for pending transactions to confirm:**
   - Check your deployer address on the explorer: https://blockscout-testnet.polkadot.io/address/YOUR_DEPLOYER_ADDRESS
...
- **"Insufficient funds"**: Make sure your deployer wallet has PAS tokens for gas
- **Network errors**: Check your internet connection and that https://services.polkadothub-rpc.com/testnet is accessible
