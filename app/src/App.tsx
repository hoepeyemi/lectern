...
// Polkadot Hub Testnet chain definition (Chain ID: 420420417)
...
const pinFileToIPFS = async (file: File): Promise<{
  ...
  try {
    ...
    const metadata = {
      name: file.name,
      description: `Uploaded via Lectern frontend`,
      attributes: {
        uploadedBy: 'Lectern',
...
// Lectern (ModredIP) contract ABI (simplified for the functions we need)
const MODRED_IP_ABI = [
  {
    inputs: [
...
export default function App({ thirdwebClient }: AppProps) {
  ...
  const loadIpAssetsViaViem = async () => {
    ...
    let nextTokenIdNum: number;
    try {
      const nextId = await polkadotHubPublicClient.readContract({
      ...
      if (nextTokenIdNum <= 0) nextTokenIdNum = 1;
    } catch {
      nextTokenIdNum = 1;
    }
    ...
    for (let i = 1; i < nextTokenIdNum; i++) {
      try {
        const result = await polkadotHubPublicClient.readContract({
          address: MODRED_IP_ADDRESS,
          abi: MODRED_IP_ABI,
          functionName: "getIPAsset",
          args: [BigInt(i)],
        });
        const owner = (result[0] ?? "").toString().toLowerCase();
        ...
        newIpAssets.set(i, {
          ...
          totalRevenue: result[6],
          royaltyTokens: result[7],
        });
      } catch {
        // skip failed or unminted
      }
    }
    setIpAssets(newIpAssets);
    const newParsed = new Map<number, any>();
    for (const [id, asset] of newIpAssets.entries()) {
      try {
        newParsed.set(id, await parseMetadata(asset.metadata));
      } catch {
        newParsed.set(id, { name: "Unknown", description: "No description available" });
      }
    }
    setParsedMetadata(newParsed);
  };
  ...
  const checkBackendStatus = async () => {
    try {
      ...
      if (!wasConnected && isConnected) {
        notifySuccess('Backend Connected', 'Successfully connected to the Lectern backend service');
      } else if (wasConnected && !isConnected) {
        notifyError('Backend Disconnected', 'Lost connection to the Lectern backend service');
      }
    } catch (error) {
      const wasConnected = backendStatus;
      ...
      if (wasConnected) {
        notifyError('Backend Error', 'Failed to connect to the Lectern backend service');
      }
    }
  };
  ...
  // Load licenses via viem public client (same Polkadot Hub RPC as IP assets for reliable reads)
  const loadLicensesViaViem = async () => {
    ...
    try {
      ...
      nextLicenseIdNum = Number(nextId);
      if (nextLicenseIdNum <= 0) nextLicenseIdNum = 1;
    } catch {
      nextLicenseIdNum = 1;
    }
    ...
    for (let i = 1; i < nextLicenseIdNum; i++) {
      try {
        const license = await polkadotHubPublicClient.readContract({
          address: MODRED_IP_ADDRESS,
          abi: MODRED_IP_ABI,
  ...
  const registerIP = async () => {
    ...
    try {
      ...
      const ipMetadata = {
        ...
        creator_email: 'creator@lectern.com', // Could be enhanced with user input
      ...
      notifySuccess('IP Asset Registered', 
        `Successfully registered IP asset!\nTransaction: ${result.polkadotHub.txHash}\nIP Asset ID: ${result.polkadotHub.ipAssetId}`,
        {
          action: {
            label: 'View Transaction',
            onClick: () => window.open(`https://blockscout-testnet.polkadot.io/tx/${result.polkadotHub.txHash}`, '_blank')
          }
        }
      );
      }
      ...
      // Reload data (licenses via Thirdweb, IP assets via viem public client)
      await loadContractData();
      await loadIpAssetsViaViem();
    ...
    try {
      ...
      notifySuccess('License Minted', 
        `Successfully minted license!\nTransaction: ${result.data.txHash}`,
        {
          action: {
            label: 'View Transaction',
            onClick: () => window.open(`https://blockscout-testnet.polkadot.io/tx/${result.data.txHash}`, '_blank')
          }
        }
      );
      } else {
  ...
  const payRevenue = async () => {
    ...
    try {
      ...
      if (royaltyBreakdown) {
        const breakdownText = [
          `Total: ${royaltyBreakdown.totalAmount} PAS`,
          `Platform Fee: ${royaltyBreakdown.platformFee.toFixed(6)} PAS (2.5%)`,
          ...royaltyBreakdown.licenseRoyalties.map(
            lr => `License ${lr.licenseId}: ${lr.amount.toFixed(6)} PAS (${lr.royaltyPercentage}%)`
          ),
          `IP Owner: ${royaltyBreakdown.ipOwnerShare.toFixed(6)} PAS`,
        ].join('\n');
  ...
  const registerArbitrator = async () => {
    ...
    try {
      setLoading(true);
      notifyInfo('Registering Arbitrator', `Registering with ${minArbitratorStake} PAS stake...`);
  ...
  const unstakeArbitrator = async () => {
    ...
    try {
      ...
      notifyInfo('Unstaking Arbitrator', `Withdrawing ${formatEther(stake)} PAS stake...`);
      ...
      notifySuccess('Stake Withdrawn', `Successfully withdrew ${formatEther(stake)} PAS! You are no longer an active arbitrator.`);
      await loadArbitrationData();
    } catch (error: any) {
  ...
  const loadArbitrationData = async () => {
    ...
    try {
      ...
      try {
        ...
        if (isExpected) {
          console.log("ℹ️ getActiveArbitratorsCount unavailable (RPC/contract). Using default 0.");
        } else {
      ...
      try {
        ...
        const isExpected =
          ...
          errorMessage.includes("No response");
        if (isExpected) {
          console.log("ℹ️ getAllArbitrators unavailable (RPC/contract). Using empty array.");
        } else {
      ...
      try {
        ...
        const isExpected =
          ...
          errorMessage.includes("No response");
        if (isExpected) {
          console.log("ℹ️ nextDisputeId unavailable (RPC/contract). Using default 1.");
        } else {
      ...
      try {
        ...
        const isExpected =
          ...
          errorMessage.includes("No response");
        if (isExpected) {
          console.log("ℹ️ Contract owner unavailable (RPC/contract). Assuming user is not the owner.");
          setIsOwner(false);
        } else {
      ...
      if (!isExpected) {
        ...
        console.log("ℹ️ Some arbitration data unavailable (RPC/contract). Continuing with defaults.");
      }
    }
  };
          ...
          <section className="testimonials-section">
            ...
            <div className="testimonials-grid">
              <div className="testimonial-card">
                ...
                <p>Lectern makes IP truly sovereign for creators—with stored licenses and creative control.</p>
              ...
              <div style={{
                ...
                <div style={{
                  ...
                  <div style={{
                    ...
                    <span style={{ color: '#1e293b' }}>{royaltyBreakdown.totalAmount} PAS</span>
                  </div>
                  ...
                  <div style={{
                    ...
                    <span style={{ color: '#1e293b' }}>{royaltyBreakdown.platformFee.toFixed(6)} PAS</span>
                  </div>
                  ...
                  <div style={{
                    ...
                    <span style={{ color: '#1e293b' }}>{royaltyBreakdown.remainingAfterFee.toFixed(6)} PAS</span>
                  </div>
                  ...
                  <div style={{
                    ...
                    <span style={{ color: '#1e293b' }}>{royaltyBreakdown.ipOwnerShare.toFixed(6)} PAS</span>
                  </div>
                      ...
                      if (isUserArbitrator && userIsActive && userStake > 0n) {
                        return (
                          <>
                            <div className="form-group" style={{ gridColumn: '1 / -1' }}>
                              <div style={{
                                ...
                                <div style={{ marginTop: '0.5rem' }}>
                                  <div>💰 Stake: {formatEther(userStake)} PAS</div>
                            ...
                            >
                              {loading ? '⏳ Unstaking...' : `💸 Unstake (${formatEther(userStake)} PAS)`}
                            </button>
                          </>
                        );
                      } else {
                        return (
                          <>
                    <div className="form-group">
                      <label className="form-label">💰 Minimum Stake (PAS)</label>
                      <input
                        type="number"
                        className="form-input"
                            ...
                            return (
                              ...
                              >
                                ...
                                <div style={{ fontSize: '0.85rem', color: 'var(--color-text-secondary)' }}>
                                  ...
                                  <div>💰 Stake: {formatEther(arb.stake)} PAS</div>
                                </div>
                              </div>
                            );
                          })}
                          ...
                          <div key={addr} className="card">
                            ...
                            <div className="card-body">
                              ...
                              <div className="card-field">
                                ...
                                <span className="card-field-value">💰 {formatEther(arb.stake)} PAS</span>
                              </div>
                              <div className="card-field">
                  ...
                  <div className="card-body">
                    ...
                    <div className="card-field">
                      ...
                      <span className="card-field-value" style={{ 
                        ...
                        💰 {parseFloat(formatEther(asset.totalRevenue)).toFixed(6)} PAS
                      </span>
                    </div>
